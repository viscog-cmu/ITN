...
class CORblock_RT(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, out_shape=None, device='cuda', omega=0.5):
        super().__init__()
...
#         self.norm_input = nn.GroupNorm(32, out_channels)
        self.conv_input = torch.nn.utils.weight_norm(
...
#         self.norm1 = nn.GroupNorm(32, out_channels)
    ...
    def forward(self, inp=None, state=None, batch_size=None):
        if inp is None:  # at t=0, there is no input yet except to V1
            inp = torch.zeros([batch_size, self.out_channels, self.out_shape, self.out_shape]).to(self.device)
        else:
